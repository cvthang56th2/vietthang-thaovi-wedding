...
// Create a new bless item using Firebase
async function createBlessItem(data) {
  try {
    // Add to Firebase Firestore
    await db.collection('blesses').add({
      name: data.name,
      description: data.description,
      // phone: data.phone || '',
      // isRegisterCar: data.isRegisterCar ? "1" : "0",
...
// Function to render bless items in real-time using Firebase
async function getAndRenderBlessItems() {
  try {
    listBless = [];
    const snapshot = await db.collection('blesses')
      .orderBy('createdAt', 'desc')
      .get();
    
    listBless = snapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data()
    }));
    ...
    console.error("Error fetching bless items:", error);
  }
}
